...
NextAuth.js is a powerful and easy to use solution for authentication users for your Next.js application.
...
##### The following guide is utilizing the following technologies:
...
Before we get started, we need to sign into our Google Account. This will allow to your [Google Cloud Console](https://console.cloud.google.com).
From there take the following steps:

1. Create a new project.
2. Then navigate to APIs & Services on the sidebar.
3. Choose CREATE CREDENTIALS and select OAuth client ID to create your application.
...
6. Set your Authorized JavaScript origins to the following URI:` http://localhost:3000`
7. Set your Authorized redirect URIs to the following: `http://localhost:3000/api/auth/callback/google`

Inside of your `.env` file, add the Client ID and Client Secret you recieved from Google:
...
5. Run the following to generate a migration, execute to the database, and trigger generation of Prisma Client.
...
You will need to grant access to your entire project to read this session.
In your `_app.tsx` file, wrap your application with [SessionProvider](https://next-auth.js.org/getting-started/upgrade-v4#sessionprovider):
...
If you have any issues, you can follow a working example within this [PrismaNext](https://github.com/Zac-Zajdel/PrismaNext).
